---
title: 【AI開発】Windows 11 + Ubuntuで環境構築（2025年）
tags:
  - CUDA
  - PyTorch
  - Poetry
  - WSL2
private: false
updated_at: '2026-03-01T14:04:29+09:00'
id: a6e24000d10adbebc242
organization_url_name: null
slide: false
ignorePublish: false
---
新しくデスクトップパソコンを購入し、環境構築を一からした際の手順をまとめました。  
忘備録です。構築したいバージョンを確認しながら、参考程度に見ることをおすすめします。  
...
本記事では、Windows 11マシン上でWSL2 (Ubuntu) を利用して、AI開発環境を構築する手順を解説します。
...
| レイヤー | コンポーネント |
| :---- | :---- |
| 1\. OS・ハードウェア層 | Windows 11, NVIDIA GPU, 最新ドライバ |
| 2\. 仮想化層 | WSL2 \+ Ubuntu 24.04 |
| 3\. システムライブラリ層 | CUDA Toolkit 12.1, cuDNN実行ライブラリ |
| 4\. プロジェクト層 | Poetry, Python 3.10, PyTorch 2.5.1 |
...
Windowsのターミナルを**管理者として実行**し、以下のコマンドでWSL2と最新のUbuntuがインストールされます。
...
これだけで、自動的にWindows側でVSCodeが起動し、WSL内のプロジェクトフォルダに接続された状態になります。Windowsの快適なUIでコードを書きながら、実行やデバッグはすべてLinux環境で行うという開発スタイルが実現できます。
